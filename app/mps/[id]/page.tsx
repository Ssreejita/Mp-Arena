...
function getSession(dateStr: string): string {
  const parts = dateStr?.split('.') || dateStr?.split('-') || [];
  ...
  if (month >= 2 && month <= 5) return `Budget ${year}`;
  if (month >= 7 && month <= 8) return `Monsoon ${year}`;
  if (month >= 11 && month <= 12) return `Winter ${year}`;
  return `Unknown ${year}`;
}
...
export default function MpDashboardPage() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto w-full">
      ...
      {/* Back Button */}
      <Link href="/mps" className="inline-flex items-center gap-1.5 text-zinc-400 hover:text-indigo-400 text-xs font-semibold transition-colors">
          ...
          <div className="flex-1 min-w-0 text-center md:text-left space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-zinc-400 text-sm flex flex-wrap items-center justify-center md:justify-start gap-3">
                <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5 text-zinc-500" />{mp.constituency}, {mp.state}</span>
                <span className="text-zinc-600">•</span>
                <span className="flex items-center gap-1"><Calendar className="h-3.5 w-3.5 text-zinc-500" />Since: {mp.start_of_term ?? '2024'}</span>
                <span className="text-zinc-600">•</span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Link href={`/mps/${id}/questions`}>
          <div className="glow-card bg-zinc-900/30 border border-zinc-900 p-5 rounded-xl flex flex-col justify-between cursor-pointer hover:border-violet-500 transition">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-zinc-400 uppercase">Questions</span>
        ...
        <Link href={`/mps/${id}/debates`}>
          <div className="glow-card bg-zinc-900/30 border border-zinc-900 p-5 rounded-xl flex flex-col justify-between cursor-pointer hover:border-pink-500 transition">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-zinc-400 uppercase">Debates</span>
        ...
        <Link href={`/mps/${id}/bills`}>
          <div className="glow-card bg-zinc-900/30 border border-zinc-900 p-5 rounded-xl flex flex-col justify-between cursor-pointer hover:border-amber-500 transition">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-zinc-400 uppercase">Bills Sponsored</span>
      ...
      <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl p-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-black text-zinc-200">Activity Timeline</h2>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(['questions', 'debates'] as const).map(t => (
              <button key={t} onClick={() => setTimelineType(t)}
                className={cn('px-3 py-1 rounded-lg text-xs font-bold capitalize transition-colors',
                  timelineType === t ? 'bg-indigo-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:text-zinc-200 border border-zinc-800'
                )}
              >{t}</button>
            ))}
            ...
            {(['year', 'session'] as const).map(v => (
              <button key={v} onClick={() => setTimelineView(v)}
                className={cn('px-3 py-1 rounded-lg text-xs font-bold capitalize transition-colors',
                  timelineView === v ? 'bg-zinc-700 text-white' : 'bg-zinc-900 text-zinc-500 hover:text-zinc-300 border border-zinc-800'
                )}
              >By {v}</button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 border-t border-zinc-900 pt-4">
          <div className="text-center">
            ...
            <p className="text-[9px] text-zinc-500 uppercase tracking-widest mt-0.5">Total Questions</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-black text-pink-400">{debates.length}</p>
            <p className="text-[9px] text-zinc-500 uppercase tracking-widest mt-0.5">Total Debates</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-black text-amber-400">{bills.length}</p>
            <p className="text-[9px] text-zinc-500 uppercase tracking-widest mt-0.5">Total Bills</p>
          </div>
        </div>
        ...
        {top5Ministries.length > 0 && (
          <div className="space-y-2 border-t border-zinc-900 pt-4">
            <p className="text-xs font-bold text-zinc-400">Top Ministries Questioned</p>
            {top5Ministries.map(([ministry, count]) => (
              <div key={ministry} className="space-y-1">
                <div className="flex justify-between text-[10px]">
                  <span className="text-zinc-400 truncate max-w-[200px]">{ministry}</span>
                  <span className="text-zinc-500 shrink-0">{count} questions</span>
                </div>
      ...
      {allMps.length > 0 && (
        <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl p-6 space-y-4">
          <div className="flex items-center gap-2">
            <Clock className="h-4 w-4 text-emerald-400" />
            <h2 className="text-sm font-black text-zinc-200">Attendance Comparison</h2>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 bg-zinc-900/20 border border-zinc-900 rounded-xl p-6 relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-zinc-300 leading-relaxed font-normal">{mp.ai_summary}</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl p-6">
          <h2 className="text-lg font-bold text-zinc-200 mb-4">Legislative Focus Areas</h2>
          <div className="space-y-4">
            {topics.map((topic) => (
              <div key={topic.id}>
        ...
        <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl p-6">
          <h2 className="text-lg font-bold text-zinc-200 mb-4">Parliamentary Activity Snapshot</h2>
          <div className="space-y-4">
            <div className="flex justify-between"><span className="text-zinc-400">Attendance</span><span className="font-semibold text-emerald-400">{mp.attendance_rate}%</span></div>
            <div className="flex justify-between"><span className="text-zinc-400">Questions Asked</span><span className="font-semibold text-violet-400">{mp.questions_count}</span></div>
            <div className="flex justify-between"><span className="text-zinc-400">Debates Participated</span><span className="font-semibold text-pink-400">{mp.debates_count}</span></div>
            <div className="flex justify-between"><span className="text-zinc-400">Bills Sponsored</span><span className="font-semibold text-amber-400">{mp.bills_sponsored}</span></div>
            <div className="border-t border-zinc-800 pt-4 mt-4">
              <div className="flex justify-between text-lg">
                <span className="font-semibold text-zinc-200">Overall Score</span>
                <span className="font-bold text-indigo-400">{mp.overall_score}/100</span>
              </div>
            </div>
          </div>
        </div>
